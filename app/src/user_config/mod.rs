...
pub(crate) fn materialize_default_worktree_config(
    ...
    // The worktree path is baked as literal text into shell `commands`, so shell-quote it to
    // keep paths containing spaces from splitting `git worktree add` / `cd` into extra
    // arguments (#11144).
    let worktree_path = crate::tab_configs::tab_config::shell_quoted_generated_worktree_path_string(
        Path::new(repo_path),
