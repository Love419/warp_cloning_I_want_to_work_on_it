...
/// Maximum raw diff output size (in bytes) that we will attempt to parse.
/// Diff output larger than this is skipped entirely — the file is returned
/// with empty hunks and [`DiffSize::Unrenderable`] to avoid multi-GB heap
/// allocations inside [`parse_diff_hunks`].
///
/// The threshold is deliberately larger than `MAX_DIFF_SIZE` (the rendering
/// limit) because parsing is cheaper than rendering, and we still want
/// `compute_diff_size` to categorise borderline diffs.  But anything above
/// 10 MB of raw diff text is almost certainly going to blow up memory.
const MAX_DIFF_PARSE_SIZE: usize = 10_000_000; // 10 MB
...
impl LocalDiffStateModel {
    ...
    async fn get_file_diff(
        ...
        // Early exit for extremely large diffs: skip the expensive parsing
        // entirely to avoid multi-GB heap allocations (see Sentry issue
        // 7259255054).  The file is still listed in the changeset but with
        // empty hunks and `Unrenderable` size so the UI can show a
        // placeholder instead of freezing.
        if diff_output.len() > MAX_DIFF_PARSE_SIZE {
            log::warn!(
                "Skipping diff parse for {file_path}: raw output is {} bytes (limit {MAX_DIFF_PARSE_SIZE})",
                diff_output.len(),
            );
            return Ok(FileDiff {
                file_path: file_path.to_owned(),
